...
- Dashboard = 15 classify rows in registry order; the 3 code rows are action-enabled, xdg/local rows
  are informational (plus the `--migrate-projects` note on code+symlink).
